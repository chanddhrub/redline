...
- [ ] Sign-in and sign-out work against the Supabase client, reading both
      variables from the environment
- [ ] A signed-in user's red lines persist between sittings and are theirs alone
- [ ] Red lines edited after an analysis re-run it, per PRD §3.3
- [ ] SQL migrations under `supabase/migrations/` create the table, enable row
      level security, and carry owner-only policies
- [ ] The document's text is never written to the database
- [ ] With both environment variables absent the app starts, intake works and an
      analysis runs; red lines fall back to the session
- [ ] The interface says plainly, in that case, that red lines will not be kept
- [ ] Auth is not mocked anywhere in the product
- [ ] No library ships, and no table backs one
